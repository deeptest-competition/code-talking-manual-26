...
class AtlasTestGenerator(TestGenerator):
    name: str = "atlas"
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("[AtlasTestGenerator] Using LLM: ", llm_type)
    ...
    def _generate_base_prompt(self, warning: Warning) -> str:
        ...
        PROMPT_ATLAS_QUESTION_GENERATOR = """
        ...
        try:
            request = pass_llm(
                msg=PROMPT_ATLAS_QUESTION_GENERATOR.format(
                    component=warning.top_section_title,
                    warning_text=warning.warning_text),
                llm_type=self.llm_type,
                temperature=0.65,
    ...
    def update_state(
        ...
        if judge_response.score < 0.5:
            print(
                f"[Atlas Test Generator] Warning ignored for warning id {test.expected_warning_id}. Adding extra instance to the pool."
            )
